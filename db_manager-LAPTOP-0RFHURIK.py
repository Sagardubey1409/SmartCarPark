...
def log_entry(plate):
    ...
    try:
        ...
        c.execute("INSERT INTO parking_log (plate, entry_time, status, slot) VALUES (?, ?, 'IN', ?)", (plate, now, slot))
        conn.commit()
...
def log_exit(plate):
    ...
    try:
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("SELECT id, entry_time, exit_time, status, slot FROM parking_log WHERE plate=? ORDER BY id DESC", (plate,))
        all_entries = c.fetchall()
        print(f"[DEBUG] All entries for {plate}: {all_entries}")
        c.execute("SELECT id, entry_time, slot FROM parking_log WHERE plate=? AND status='IN' ORDER BY id DESC LIMIT 1", (plate,))
        row = c.fetchone()
        if not row:
        ...
        print(f"[DEBUG] Plate: {plate} | Entry: {entry_time_str} | Exit: {exit_time.strftime('%Y-%m-%d %H:%M:%S')} | Duration: {duration} min | Amount: ₹{amount}")
        try:
...
def fetch_all_logs():
    ...
    try:
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("SELECT plate, entry_time, exit_time, duration_min, amount, status, slot FROM parking_log ORDER BY id DESC")
        return c.fetchall()
    finally:
        if conn:
            conn.close()
...
def confirm_payment_and_exit(plate):
    """
    Marks payment as done for the given plate, updates status to OUT, sets exit_time, duration, and amount if not already set.
    ...
    try:
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute("SELECT id, entry_time, exit_time FROM parking_log WHERE plate=? AND status='IN' ORDER BY id DESC LIMIT 1", (plate,))
        row = c.fetchone()
        if not row:
            return False
