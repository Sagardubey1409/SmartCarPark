...
# Simple in-memory pending exit plate (thread-safe)
...
def dashboard():
    ...
    try:
        ...
        return render_template('dashb.html', logs=logs, slots=slots)
    except Exception as e:
        logger.error(f"Dashboard error: {e}")
        return render_template('dashb.html', logs=[], slots={}, error=str(e))
...
# SMS Gateway webhook integration (configurable)
...
def check_status(plate):
    """Check parking status for a specific plate"""
    try:
        conn = None
        try:
...
def exit_page(plate):
    """Generate exit page with QR code for payment"""
    try:
...
# Update the db_manager functions to emit events
...
def emit_exit_event(plate, exit_info):
    # Normalize dataclass ExitResult to dict if necessary
    if is_dataclass(exit_info):
        info = asdict(exit_info)
    elif isinstance(exit_info, dict):
